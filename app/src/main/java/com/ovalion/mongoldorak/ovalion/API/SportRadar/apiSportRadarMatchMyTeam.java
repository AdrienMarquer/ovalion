...
public class apiSportRadarMatchMyTeam extends AsyncTask<Void, Void, List<Match>> {
    private List<Match> matchs = new ArrayList<>();
    private TeamFragment frag;
    private String id;

    public apiSportRadarMatchMyTeam(TeamFragment frag, String id)
    {
        this.frag = frag;
        this.id = id;
    }
    ...
    public List<Match> request() {
        try {
            ...
            for (int i = 0; i < list_match.length(); i++) {
                ...
                if ((data.getCompetitorA().getId().contains(id) ||  data.getCompetitorB().getId().contains(id)) && pastMatchCheck(data.getDate())){
                    list.add(data);
                }
            }
            return list;
    ...
    private boolean pastMatchCheck(String date){
